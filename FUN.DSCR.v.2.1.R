...
FUN.cr <- function(data, kluster, Beta, sdd, contcov, catcov, n.poly, 
  ...
  # Calculate entropy (measure of classification uncertainty)
  entropy <- -sum(T * log(T + 1e-16), na.rm = TRUE)  # small constant to avoid log(0)
  normalized_entropy <- entropy / (nrow(T) * log(kluster))  # normalized between 0 and 1
  ...
  return(list(
    ...
    aic = aic,
    bic = bic,
    icl = icl,                   # Integrated Completed Likelihood
    entropy = entropy,           # Classification entropy
    normalized_entropy = normalized_entropy,  # Normalized entropy
    n_clusters = kluster,
...
FUN.select_clusters <- function(data, max_clusters = 5, n.poly=3, equal.variance=TRUE, verbose = TRUE, ...) {
  ...
  icl_values <- numeric(max_clusters)  # Added for ICL
  ...
  for (k in 1:max_clusters) {
    ...
    if (!is.null(cr_result)) {
      ...
      icl_values[k] <- bic_values[k]  # For k=1, ICL = BIC (no clustering)
    }
  }
  
  # Find optimal number based on BIC and ICL
  optimal_k_bic <- which.min(bic_values)
  optimal_k_icl <- which.min(icl_values)
  ...
  if (verbose) {
    ...
    print(data.frame(
      ...
      ICL = round(icl_values, 2),  # Added ICL column
      marg_LLK = round(marg_llk_values, 2)
    ))
    message("\nOptimal number of clusters:")
    message(" - By BIC: ", optimal_k_bic)
    message(" - By ICL: ", optimal_k_icl)
  }
  ...
  return(list(
    ...
    icl_values = icl_values,  # Added ICL to output
    marg_llk_values = marg_llk_values,
    optimal_k_bic = optimal_k_bic,
    optimal_k_icl = optimal_k_icl,
    recommended_k = ifelse(!is.na(optimal_k_bic), optimal_k_bic, optimal_k_icl)
  ))
}